...
public class TesseractOcr implements AutoCloseable {
    ...
    public TesseractOcr() {
        ...
        this.handle = com.sun.jna.Pointer.nativeValue(TessAPI.getInstance().TessBaseAPICreate());
        if (this.handle == 0) {
    ...
    public synchronized void init() {
        ...
        int result = TessAPI.getInstance().TessBaseAPIInit3(
        ...
        TessAPI.getInstance().TessBaseAPISetPageSegMode(getHandle(), pageSegMode);
        initialized = true;
    }
    ...
    public String doOCR(byte[] imageData, int width, int height, int channels) {
        ...
        TessAPI.getInstance().TessBaseAPISetImage(
            handlePtr, imageData, width, height, channels, bytesPerLine
        );

        com.sun.jna.Pointer textPtr = TessAPI.getInstance().TessBaseAPIGetUTF8Text(handlePtr);
        if (textPtr == null) {
            return "";
        }
        String result = textPtr.getString(0);
        TessAPI.getInstance().TessDeleteText(textPtr);
        return result;
    }
    ...
    public void close() {
        if (handle != 0) {
            TessAPI.getInstance().TessBaseAPIDelete(getHandle());
            handle = 0;
            initialized = false;
        }
    }
