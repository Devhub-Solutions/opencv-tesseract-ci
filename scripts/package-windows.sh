...
if [ -d "${ARTIFACT_DIR}/opencv" ]; then
    ...
    cp -v "${ARTIFACT_DIR}/opencv/"*.jar "${ARTIFACT_DIR}/opencv-upload/" 2>/dev/null || true
    cp -v "${ARTIFACT_DIR}/opencv/"*.dll "${ARTIFACT_DIR}/opencv-upload/" 2>/dev/null || true
    cp -v "${ARTIFACT_DIR}/opencv/include/"*.h "${ARTIFACT_DIR}/opencv-upload/" 2>/dev/null || true
    # FIX: Copy DLL dependencies from deps/
    if [ -d "${ARTIFACT_DIR}/opencv/deps" ]; then
        cp -v "${ARTIFACT_DIR}/opencv/deps/"*.dll "${ARTIFACT_DIR}/opencv-upload/" 2>/dev/null || true
    fi
fi
...
if [ -d "${ARTIFACT_DIR}/tesseract" ]; then
    ...
    cp -v "${ARTIFACT_DIR}/tesseract/bin/"*.dll "${ARTIFACT_DIR}/tesseract-upload/" 2>/dev/null || true
    cp -v "${ARTIFACT_DIR}/tesseract/lib/"*.dll.a "${ARTIFACT_DIR}/tesseract-upload/" 2>/dev/null || true
    cp -v -r "${ARTIFACT_DIR}/tesseract/include/tesseract" "${ARTIFACT_DIR}/tesseract-upload/" 2>/dev/null || true
fi
